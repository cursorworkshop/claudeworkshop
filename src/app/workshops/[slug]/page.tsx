...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `/workshops/${event.slug}`,
    },
    openGraph: {
      title: event.title,
      description: event.description,
...
export default async function WorkshopPage({ params }: EventPageProps) {
  const { slug } = await params;
  const event = await getEventBySlug(slug);
  ...
  // For the Mani workshop, show the stunning details page
  if (slug === '2025-10-20-claude-deep-dive') {
    return <StunningEventDetails />;
  }

  // For other events, show the regular event page
