...
const createRays = (count: number, cycle: number): LightRay[] => {
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    const swing = 2 + Math.random() * 4;
    const delay = Math.random() * cycle;
    const duration = cycle * (0.6 + Math.random() * 0.5);
      ...
      <div className='absolute inset-0 overflow-hidden'>
        <div
          ...
          style={
            {
              background:
                'radial-gradient(circle at 20% 15%, color-mix(in srgb, var(--light-rays-color) 60%, transparent), transparent 65%)',
            } as CSSProperties
          }
        />
        <div
          ...
          style={
            {
              background:
                'radial-gradient(circle at 80% 10%, color-mix(in srgb, var(--light-rays-color) 50%, transparent), transparent 70%)',
            } as CSSProperties
          }
        />
        {rays.map(ray => (
