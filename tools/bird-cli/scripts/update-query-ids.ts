...
const OPERATION_PATTERNS = [
  {
    regex:
      /operationName\s*[:=]\s*["']([^"']+)["']([\s\S]{0,4000}?)queryId\s*[:=]\s*["']([^"']+)["']/g,
    operationGroup: 1,
    queryIdGroup: 3,
  },
  {
    regex:
      /queryId\s*[:=]\s*["']([^"']+)["']([\s\S]{0,4000}?)operationName\s*[:=]\s*["']([^"']+)["']/g,
    operationGroup: 3,
    queryIdGroup: 1,
  },
] as const;
...
async function writeIds(ids: Record<OperationName, string>): Promise<void> {
  const ordered: Record<OperationName, string> = {} as Record<
    OperationName,
    string
  >;
  for (const op of TARGET_OPERATIONS) {
...
async function main(): Promise<void> {
  ...
  if (discovered.size === 0) {
    throw new Error(
      'No query IDs discovered; extraction patterns may need an update.'
    );
  }
