...
program.addHelpText(
  ...
  () =>
    `\n${colors.section('Examples')}\n${[
      ...
      formatExample('bird bookmarks --json -n 20', 'Fetch your latest bookmarks as JSON'),
...
program
  .command('bookmarks')
  .description('Fetch bookmarked tweets for the authenticated account')
  .option('-n, --count <number>', 'Number of bookmarks to fetch', '20')
  .option('--all', 'Fetch all available bookmarks up to --max-pages')
  .option('--max-pages <number>', 'Maximum pages when using --all (100 per page)', '10')
  .option('--json', 'Output as JSON')
  .action(async (cmdOpts: { count?: string; all?: boolean; maxPages?: string; json?: boolean }) => {
    const opts = program.opts();
    const count = Number.parseInt(cmdOpts.count || '20', 10);
    const maxPages = Math.max(1, Number.parseInt(cmdOpts.maxPages || '10', 10));
    const requestedCount = cmdOpts.all ? maxPages * 100 : count;
    const sweetistics = resolveSweetisticsConfig({
      sweetisticsApiKey: opts.sweetisticsApiKey || config.sweetisticsApiKey,
      sweetisticsBaseUrl: opts.sweetisticsBaseUrl || config.sweetisticsBaseUrl,
    });
    ...
    if (useSweetistics) {
      console.error('❌ Bookmarks are currently supported only via GraphQL cookie auth.');
      process.exit(1);
    }
    ...
    const client = new TwitterClient({ cookies });
    const result = await client.getBookmarks(Math.max(1, requestedCount));
    ...
    console.error(`❌ Failed to fetch bookmarks: ${result.error ?? 'Unknown error'}`);
    process.exit(1);
  });
