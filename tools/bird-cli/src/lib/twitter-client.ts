...
const BOOKMARK_MATCH_ALL_QUERY = 'filter:links OR -filter:links';
...
const FALLBACK_QUERY_IDS = {
  ...
  BookmarkSearchTimeline: 'xyLBeQ_-k7meeIfobisRHg',
} as const;
...
type TimelineInstructions = TimelineInstruction[];
...
export class TwitterClient {
  ...
  private findTweetInInstructions(instructions: TimelineInstructions | undefined, tweetId: string) {
  ...
  private mapTweetResult(result: GraphqlTweetResult | undefined): TweetData | undefined {
    ...
    const maybeUsername = userResult?.legacy?.screen_name || userResult?.core?.screen_name;

    if (!result?.legacy || !maybeUsername) return undefined;
    const username = maybeUsername;
    const displayName = userResult?.legacy?.name || userResult?.core?.name || username;
    return {
      id: result.rest_id || '',
  ...
  private parseTweetsFromInstructions(instructions: TimelineInstructions | undefined): TweetData[] {
    ...
    for (const instruction of instructions ?? []) {
      for (const entry of instruction.entries ?? []) {
        ...
        // Some URT timelines include tweets inside module items.
        const moduleItems = entry.content?.items;
        if (Array.isArray(moduleItems)) {
          for (const moduleItem of moduleItems) {
            const moduleResult = moduleItem.item?.itemContent?.tweet_results?.result;
            const moduleMapped = this.mapTweetResult(moduleResult);
            if (moduleMapped) tweets.push(moduleMapped);
          }
        }
      }
    }

    const deduped = new Map<string, TweetData>();
    for (const tweet of tweets) {
      if (tweet.id && !deduped.has(tweet.id)) {
  ...
  private findBottomCursor(instructions: TimelineInstructions | undefined): string | undefined {
    for (const instruction of instructions ?? []) {
      for (const entry of instruction.entries ?? []) {
        const cursorType = entry.content?.cursorType;
        const value = entry.content?.value;
        if (cursorType === 'Bottom' && typeof value === 'string' && value.length > 0) {
          return value;
        }
      }
    }
  ...
  async getBookmarks(count = 20): Promise<SearchResult> {
    const pageLimit = Math.max(1, Math.min(100, count));
    const maxPages = Math.max(1, Math.ceil(count / pageLimit));
    ...
    const dedupedTweets = new Map<string, TweetData>();
    let cursor: string | undefined;
    let pagesFetched = 0;

    while (pagesFetched < maxPages && dedupedTweets.size < count) {
      const variables: Record<string, string | number> = {
        // BookmarkSearchTimeline now requires a non-empty rawQuery.
        // This tautology matches both tweets with and without links, effectively "all bookmarks".
        rawQuery: BOOKMARK_MATCH_ALL_QUERY,
        count: pageLimit,
      };
      if (cursor) {
      ...
      try {
        ...
        const pageTweets = this.parseTweetsFromInstructions(instructions);
        for (const tweet of pageTweets) {
          if (tweet.id && !dedupedTweets.has(tweet.id)) {
            dedupedTweets.set(tweet.id, tweet);
          }
        }

        cursor = this.findBottomCursor(instructions);
        pagesFetched += 1;

        if (!cursor || pageTweets.length === 0) {
          break;
        }
      } catch (error) {
        return {
    ...
    return {
      success: true,
      tweets: [...dedupedTweets.values()].slice(0, count),
    };
  }
